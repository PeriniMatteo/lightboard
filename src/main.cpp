...
Screen *s;
pthread_t main_tid;
/* wait_time indicates how much ms to wait between IR signals
** this is necessary because xwiimote has no method to know when IR data stops
** so we define a value and wait for data, if the timer expires, we consider the IR pen as off
** It is possible to change this value from the configuration window */
int point, change, wait_time;
coord wii_coord[4];
...
static char *get_dev(int num){
    ...
    if (!ent)
    printf("Cannot find device with number #%dn", num);
    return ent;
}
...
int main(int argc, char *argv[]) {
    ...
    free(path);
    if (ret) printf("Cannot create xwii_iface '%s' err:%dn", argv[1], ret);
    else {
        ...
        if (signal(SIGUSR1, sig_handler) == SIG_ERR) printf("can't catch SIGUSR1n");
        if (signal(SIGUSR2, sig_handler) == SIG_ERR) printf("can't catch SIGUSR2n");
